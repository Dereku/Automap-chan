...
public class NoteArray {
    ...
    private final ArrayList<Note> BGnotes = new ArrayList<>();;
    ...
    public NoteArray clone() {
        ...
        result.notes.addAll((ArrayList<Note>) notes.clone());
     ...
     * as Note n
     *
     * @param n Note object to search for
    ...
    public boolean contains(Note n, int tempo) {
        ...
        for (Note note : notes) {
            ...
            if (note.getName().equals(n.getName()) && note.getBPM() == tempo) {
                return true;
            }
    ...
    public void sortByName() {
        Collections.sort(notes, Note.NOTE_NAME_COMPARATOR);
    }

    public void sortByDuration() {
        Collections.sort(notes, Note.DURATION_COMPARATOR);
    }

    public void sortByPitch() {
        Collections.sort(notes, Note.PITCH_COMPARATOR);
    }
    ...
    public ArrayList<NoteArray> sortNotesByTime() {
        ...
        Collections.sort(notes, Note.START_TIME_COMPARATOR);
        ...
        while (ite.hasNext()) {
            ...
            if (previousT == -1) {
                ...
                previousT = n.getAbs();
                temp.add(n);
            } else // for every other iterations
    ...
    public ArrayList<NoteArray> sortBGNotesByTime() {
        // sort the whole array by its start time
        Collections.sort(BGnotes, Note.START_TIME_COMPARATOR);
        ArrayList<NoteArray> output = new ArrayList<>();
    ...
    public int notePitchCompare(NoteArray Chord) {
        ...
        ArrayList<Note> currentNoteSet = new ArrayList<>();
        ArrayList<Note> otherNoteSet = new ArrayList<>();
        ...
        Collections.sort(currentNoteSet, Note.PITCH_COMPARATOR);
        Collections.sort(otherNoteSet, Note.PITCH_COMPARATOR);
        Collections.sort(currentNoteSet, Note.CHANNEL_COMPARATOR);
        Collections.sort(otherNoteSet, Note.CHANNEL_COMPARATOR);
